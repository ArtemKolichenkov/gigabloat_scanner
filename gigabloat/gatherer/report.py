...
class ReportPrinter:
    ...
    def prepare_directories(self):
        table = [[dir.path, f"{dir.size} ({dir.hr_size})"] for dir in self.scanner.directories]
    ...
    def prepare_type_stats(self):
        sorted_filetypes = sorted(self.scanner.filetypes.items(), key=lambda item: item[1], reverse=True)
        sorted_stats = [['no extension' if ext == '' else ext, count] for ext, count in sorted_filetypes]
        table_print = tabulate(sorted_stats, headers=FILETYPE_HEADERS, tablefmt="fancy_grid")
        return make_title("File types") + table_print

    def prepare_category_stats(self):
        sorted_categories = sorted(self.scanner.categories.items(), key=lambda item: item[1], reverse=True)
        sorted_stats = [[cat, count] for cat, count in sorted_categories]
        table_print = tabulate(sorted_stats, headers=CATEGORY_HEADERS, tablefmt="fancy_grid")
    ...
    def prepare_no_extension_files(self):
        ...
        table_print = tabulate(tab_noext_files, headers=NOEXT_HEADERS, tablefmt="fancy_grid")
    ...
    def prepare_root_stats(self):
        ...
        table = [[self.scanner.root, self.scanner.file_count,
                  self.scanner.dir_count, self.scanner.hr_size, self.scanner.size]]
    ...
    def prepare_all_files(self):
        all_files_headers = [f"File ({self.scanner.file_count})", 'Size', 'Size (bytes)']
        tab_files = [[f.name, f.hr_size, f.size] for f in self.scanner.files]
        table_print = tabulate(tab_files, headers=all_files_headers, tablefmt="fancy_grid")
